...
public class ImageUtilities {
    public static class ImageData {
        ...
        public ImageData(ByteBuffer buffer, int width, int height) {
            this.buffer = buffer;
            this.width = width;
            this.height = height;
        }
    ...
    public static ImageData getImageData(String path) {
        ...
        try (MemoryStack stack = MemoryStack.stackPush()) {
            ...
            return new ImageData(buffer, w.get(), h.get());
        } catch (Exception e) {
            System.err.println("Error reading image \"" + path + "\".");
        }
        return new ImageData(null, 0, 0);
    }

    public static void freeImageBuffer(ByteBuffer buffer) {
        stbi_image_free(buffer);
    }


    /**
     * Please note that all code below is from TWL's PNGDecoder library.
     */
